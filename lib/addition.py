...
def adding_3_numbers(first_no, second_no, third_no):
    """
...
def adding_n_numbers(*numbers):
    ...
    if len(numbers) > 1:
        ...
        if len(numbers) == 1:
            # sum_of_n_numbers = numbers[0]
            # result = numbers   # Simply we can write
            print(
                f"Single Number {sum_of_n_numbers} supplied.. Hence returning the Same NO.!")
            return sum_of_n_numbers
        else:
...
def adding_required_numbers():
    ...
    while counter < how_many_numbers:
        result_sum += int(input("Enter the Number-" +
                                str(counter + 1) + " : "))
        counter += 1
    return result_sum
